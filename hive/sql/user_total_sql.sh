sql="select count(*),sum(tosize),count(distinct actor), actor%100 from stat_dispatch where dt='$1' and mcdispatcher='' and "
...
while read l; do
        ...
        statsql="insert into feed_stat_user_total(date, dispatch, tosize, dispatch_user) values('$1', $c1, $c2, $c3) on duplicate key update dispatch=values(dispatch), tosize=values(tosize), dispatch_user=values(dispatch_user)"
